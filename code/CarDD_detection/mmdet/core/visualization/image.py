...
def draw_masks(ax, img, masks, color=None, with_edge=True, alpha=0.8):
    ...
    p = PatchCollection(
        ...
        polygons, facecolor='none', edgecolors=new_color, linewidths=5) # mask style
    ax.add_collection(p)
...
def imshow_det_bboxes(img,
    ...
    """
    if isinstance(bboxes, DataContainer):
        bboxes = bboxes.data
        bboxes = bboxes.numpy()
    ...
    if bboxes is not None:
        ...
        bbox_palette = ['#FFB6C1', '#00a8e1', '#00FF00', '#800080', '#FFFF00', '#e30039'] # bbox color
        colors = [bbox_palette[label] for label in labels[:num_bboxes]]
        # draw_bboxes(ax, bboxes, colors, alpha=0.8, thickness=thickness)
        draw_bboxes(ax, bboxes, colors, alpha=1, thickness=5) # bbox style
    ...
    if segms is not None:
        ...
        mask_palette = [(255, 182, 193), (0, 168, 225), (0, 255, 0), (128, 0, 128), (255, 255, 0), (227, 0, 57)] # change color
        colors = [mask_palette[label] for label in labels]
        colors = np.array(colors, dtype=np.uint8)
        # draw_masks(ax, img, segms, colors, with_edge=True)
        draw_masks(ax, img, segms, colors, with_edge=True, alpha=0.5) # mask style
